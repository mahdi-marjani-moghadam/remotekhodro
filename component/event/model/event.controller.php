...
class eventController
{
    ...
    public function showDetail($id)
    {
        ...
        $export['list']['city'] = $obj->fields['name'];
    ...
    public function showALL($fields)
    {
        ...
        //echo "<pre>";
        //print_r($calendar['export']['list']);
        foreach ($calendar['export']['list'] as $k => $v)
        {
            $temp[$k]['title'] = $v["event_name_$lang"];
            $temp[$k]['start'] = $v["date"];
            $temp[$k]['color'] = '#ffad33';
            $temp[$k]['url'] = RELA_DIR . 'event/Detail/' . $v['Event_id'] . '/' . $v["event_name_$lang"];
        }
        $export['calendar'] = json_encode($temp);
//        print_r_debug(json_encode($temp));
